...
pub struct AppConfig {
    ...
    #[serde(default)]
    pub auto_open_in_editor: bool,    // Automatyczne otwieranie zmienionych plików w edytorze IDE (domyślnie false)
}
...
impl Default for AppConfig {
    fn default() -> Self {
        Self {
            ...
            sync_auto: false,
            ide_mode: false,
            auto_open_in_editor: false,
        }
    }
}

impl AppConfig {
    /// Konfiguracja bez side-effectów sieci/serwera — do testów jednostkowych.
    #[cfg(test)]
    pub fn for_tests() -> Self {
        let mut cfg = Self::default();
        cfg.web_companion_enabled = false;
        cfg
    }

    /// Ścieżka do globalnego pliku konfiguracyjnego (~/.opencode-rs/config.json)
    pub fn config_path() -> PathBuf {
        // 0. Tryb przenośny (obok opencode.exe)
        ...
        // 1. Dedykowany katalog domowy ~/.opencode-rs/config.json (priorytet)
        if let Some(user_dirs) = directories::UserDirs::new() {
            let rs_dir = user_dirs.home_dir().join(".opencode-rs");
            std::fs::create_dir_all(&rs_dir).ok();
            return rs_dir.join("config.json");
        }
        // 2. Fallback ProjectDirs
    ...
    /// Ścieżka do lokalnej konfiguracji projektu (.opencode-rs/config.json z fallbackiem do .opencode/config.json)
    pub fn project_config_path(work_dir: &Path) -> PathBuf {
        let rs_path = work_dir.join(".opencode-rs").join("config.json");
        if rs_path.exists() {
            return rs_path;
        }
        let legacy = work_dir.join(".opencode").join("config.json");
        if legacy.exists() {
            return legacy;
        }
        rs_path
    ...
    pub fn load_for_project(work_dir: &Path) -> Self {
        ...
        // Sprawdź czy projekt ma lokalny plik konfiguracyjny
    ...
    pub fn save_project_config(&self, work_dir: &Path) -> anyhow::Result<()> {
        let local_path = work_dir.join(".opencode-rs").join("config.json");
