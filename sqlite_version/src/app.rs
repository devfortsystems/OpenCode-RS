...
pub struct App {
    ...
    /// Cache metadanych środowiska i tokenów (dla płynnego renderowania 0ms opóźnienia)
...
impl App {
    pub fn new(work_dir: PathBuf, config: AppConfig) -> Self {
        ...
        // W testach pomijamy — tokio::test czeka na spawny, a `opencode models` / Web Companion
        // potrafią wisieć w nieskończoność i zabić cały `cargo test`.
        #[cfg(not(test))]
        {
        ...
        let cached_branch = crate::environment::EnvironmentManager::get_current_branch(&work_dir).unwrap_or_else(|_| "main".to_string());
        let cached_stack = agent.context().detect_project_stack();
    ...
    pub fn refresh_tokens_and_cost(&mut self) {
        ...
        self.cached_tokens_and_cost = (est_tokens, max_tokens, percent, cost_str);
    }

    /// Zwraca stokenizowany i skalkulowany koszt z pamięci podręcznej (0ms opóźnienia, zero I/O)
    pub fn estimate_tokens_and_cost(&self) -> (usize, usize, usize, String) {
        self.cached_tokens_and_cost.clone()
    }
    ...
    pub fn handle_app_event(&mut self, event: AppEvent) {
        match event {
            ...
            AppEvent::FileModified(path) => {
                if self.config.auto_open_in_editor {
...
mod tests {
    ...
    fn fresh_app() -> App {
        ...
        App::new(dir, AppConfig::for_tests())
    }
