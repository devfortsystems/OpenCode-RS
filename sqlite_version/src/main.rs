...
enum Command {
    ...
    Models {
        /// Filtruj po providerze (np. cursor, windsurf, antigravity, gemini, openai)
        provider: Option<String>,
        /// Pokaż tylko w 100% darmowe modele (0.00$)
        #[arg(long)]
        free: bool,
    ...
    /// Stan wszystkich pakietów, limitów zapytań i dat odnowienia
    #[command(alias = "quotas")]
    Quota {
        /// Wyjście JSON (machine-readable)
        #[arg(long)]
        json: bool,
...
async fn main() -> Result<()> {
    ...
    // Automatyczna konsolidacja i utrwalenie wszystkich kluczy w ~/.opencode-rs/
    let _ = AuthManager::persist_credentials(&config, &work_dir);
    ...
    if args.tray {
        ...
        return crate::tray::run_tray_icon(port, work_dir);
    }
    ...
    if args.web {
        ...
        let (tx, _rx) = tokio::sync::mpsc::channel(10);
        companion.start_background(tx);
        crate::tray::open_browser(port);
        println!("🚀 [OpenCode-RS] Web UI aktywny pod adresem: http://127.0.0.1:{}", port);
        println!("Naciśnij Ctrl+C, aby zatrzymać serwer.");
...
async fn run_subcommand(cmd: &Command, work_dir: &Path, args: &CliArgs) -> Result<()> {
    match cmd {
        ...
        Command::Models { provider, free, json } => {
            run_models_list(work_dir, provider.as_deref(), *free, *json).await
        }
        ...
        Command::Quota { json } => {
            run_quota_cmd(work_dir, *json).await
        }
        ...
        Command::Tray { port } => {
            ...
            let (tx, _rx) = tokio::sync::mpsc::channel(10);
            companion.start_background(tx);
            crate::tray::run_tray_icon(*port, work_dir.to_path_buf())
        }
...
/// `opencode quota [--json]` — stan pakietów, limitów i sald
async fn run_quota_cmd(work_dir: &Path, json: bool) -> Result<()> {
    AuthManager::auto_load_credentials(work_dir);
    let auth_keys = AuthManager::get_active_keys();
    let quotas = crate::quota::QuotaManager::check_all_quotas(&auth_keys, 0).await;
    ...
    println!("\n📊 Stan pakietów, limitów i subskrypcji providerów AI ({})", quotas.len());
    println!("{:<28} {:<24} {:<20} {:<22} {:<12}", "PROVIDER", "PLAN / TYP", "WYKORZYSTANE", "RESET / ODNOWIENIE", "STATUS");
    println!("{}", "─".repeat(110));

    for q in &quotas {
        let usage_str = match (q.quota_total, q.percentage_used) {
            (Some(tot), Some(pct)) => format!("{:.0}/{:.0} {} ({:.1}%)", q.quota_used, tot, q.unit, pct),
            _ => format!("{:.2} {}", q.quota_used, q.unit),
        };
        let reset_str = q.resets_at.as_deref().unwrap_or("—");
        let status_badge = match q.status.as_str() {
            "connected" | "active" => "🟢 AKTYWNY",
            "warning" => "🟡 UWAGA (>80%)",
            "depleted" => "🔴 WYCZERPANY",
            "no_key" => "⚪ BRAK KLUCZA",
            _ => "⚪ OFFLINE",
        };
        println!("{:<28} {:<24} {:<20} {:<22} {:<12}", q.name, q.plan_type, usage_str, reset_str, status_badge);
    }
    println!("{}", "─".repeat(110));
    println!("💡 Pełne dane z wykresami dostępne w Web UI: opencode web -> zakładka Quota & Subscriptions\n");

    Ok(())
}

/// `opencode models [provider] [--free] [--json]` — lista tylko DZIAŁAJĄCYCH modeli u wszystkich providerów z cenami
async fn run_models_list(work_dir: &Path, provider_filter: Option<&str>, free_only: bool, json: bool) -> Result<()> {
    AuthManager::auto_load_credentials(work_dir);
    let config = AppConfig::load_for_project(work_dir);
    let router = Arc::new(ProviderRouter::new(config.clone(), work_dir.to_path_buf()));

    // 1. Sprawdź dostępność poszczególnych providerów (tylko działające!)
    ...
    // 3. Filtruj TYLKO działające modele
    ...
    // 4. Mapuj na ModelCatalogItem z pełnym cennikiem i statusem
    ...
    // 5. Filtrowanie opcjonalne (free / provider)
    if free_only {
        catalog_items.retain(|item| item.is_free);
    }
    if let Some(f) = provider_filter {
        let fl = f.to_lowercase();
        catalog_items.retain(|item| {
            item.id.to_lowercase().contains(&fl)
                || item.name.to_lowercase().contains(&fl)
                || item.provider.to_lowercase().contains(&fl)
                || item.provider_label.to_lowercase().contains(&fl)
        });
    }

    // Sortuj: najpierw darmowe 0.00$, potem rosnąco po cenie
    catalog_items.sort_by(|a, b| {
        a.cost_per_1m_input.partial_cmp(&b.cost_per_1m_input).unwrap_or(std::cmp::Ordering::Equal)
    });

    if json {
        println!("{}", serde_json::to_string_pretty(&catalog_items)?);
        return Ok(());
    }

    if catalog_items.is_empty() {
        println!("❌ Brak działających modeli{}.", provider_filter.map(|f| format!(" dla filtra '{f}'")).unwrap_or_default());
        println!("💡 Uruchom 'opencode doctor', aby sprawdzić brakujące klucze API i binarki.");
        return Ok(());
    }

    println!("\n🚀 Dostępne i działające modele w OpenCode-RS ({})", catalog_items.len());
    println!("{:<32} {:<24} {:<24} {:<12}", "MODEL (ID)", "DOSTAWCA (PROVIDER)", "CENA (1M TOKENÓW)", "STATUS / TYP");
    println!("{}", "─".repeat(96));

    for item in &catalog_items {
        let badge = if item.is_free {
            "🟢 DARMOWY"
        } else {
            "💳 PŁATNY"
        };
        println!("{:<32} {:<24} {:<24} {:<12}", item.id, item.provider_label, item.price_display, badge);
    }
    println!("{}", "─".repeat(96));
    println!("💡 Aby użyć dowolnego modelu: opencode -m <id> lub wybierz go w karcie Web UI.");
    println!("💡 Filtry CLI: opencode models --free (tylko darmowe), opencode models <provider>, opencode models --json");
    println!("💡 Podgląd zużycia pakietów: opencode quota\n");
...
fn run_auth_cmd(work_dir: &Path, sub_args: &[String]) -> Result<()> {
    ...
    match action {
        ...
        "add" | "set" => {
            if sub_args.len() >= 3 {
                let provider = &sub_args[1];
                let key = &sub_args[2];
                if let Err(e) = AuthManager::set_credential(provider, key, work_dir) {
                    eprintln!("❌ Błąd zapisu: {e}");
                } else {
                    println!("✅ Klucz dla providera '{}' zapisany w ~/.opencode-rs/auth.json oraz .env", provider);
                }
            } else {
                println!("Użycie: opencode auth add <provider> <klucz>");
                println!("Przykłady:");
                println!("  opencode auth add gemini AIzaSy...");
                println!("  opencode auth add openai sk-proj-...");
                println!("  opencode auth add anthropic sk-ant-...");
                println!("  opencode auth add groq gsk_...");
                println!("  opencode auth add deepseek sk-...");
            }
        }
        "remove" => {
            if sub_args.len() < 2 {
            ...
            println!("Usunięcie providera '{}' — edytuj ~/.opencode-rs/auth.json", sub_args[1]);
        }
        _ => {
            println!("Użycie: opencode auth [status|list|add <provider> <key>|remove <name>]");
        }
    }
    Ok(())
}
...
fn run_doctor(work_dir: &Path, json: bool) -> Result<()> {
    ...
    // 5. DevFortDB
    let db_path = crate::database::Database::resolve_db_dir(work_dir).join("opencode.mdb");
