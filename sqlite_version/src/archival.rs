...
/// Węzeł w indeksie wektorowym pamięci archiwalnej.
...
/// Lekki indeks wektorowy in-memory z metryką podobieństwa cosinusowego (dla SQLite version).
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
pub struct SimpleVectorIndex {
    nodes: std::collections::HashMap<String, VectorNode>,
}
...
impl SimpleVectorIndex {
    ...
    pub fn put_node(&mut self, id: String, vector: Option<Vec<f32>>, labels: Vec<String>) {
        self.nodes.insert(id.clone(), VectorNode { id, vector, labels });
    }

    pub fn add_node(&mut self, node: VectorNode) {
        self.nodes.insert(node.id.clone(), node);
    }

    pub fn nodes(&self) -> &std::collections::HashMap<String, VectorNode> {
        &self.nodes
    }

    pub fn node_count(&self) -> usize {
        self.nodes.values().filter(|n| n.vector.is_some()).count()
    }

    pub fn vector_search(&self, query_vec: &[f32], top_k: usize) -> Vec<(String, f32)> {
        ...
        let mut scored: Vec<(String, f32)> = self
            ...
            .values()
            .filter_map(|node| {
                let vec = node.vector.as_ref()?;
                if vec.len() != query_vec.len() {
                    return None;
                }
                let dot: f32 = query_vec.iter().zip(vec.iter()).map(|(a, b)| a * b).sum();
                let v_norm = vec.iter().map(|v| v * v).sum::<f32>().sqrt();
...
/// Archival memory — wrapper na SimpleVectorIndex + SQLite Database.
pub struct ArchivalMemory {
    db: Database,
    /// In-memory cache wektorów (ładowany z DB przy pierwszym użyciu).
    gv: Arc<Mutex<SimpleVectorIndex>>,
    /// Embedding provider.
    embedder: Embedder,
}
...
impl ArchivalMemory {
    ...
    /// Ładuje indeks wektorowy z DB.
    fn load_graph(db: &Database) -> Result<SimpleVectorIndex> {
        if let Ok(Some(bytes)) = db.get_str::<Vec<u8>>("archival", "graph") {
            if let Ok(gv) = SimpleVectorIndex::from_bytes(&bytes) {
                return Ok(gv);
            }
        }
        Ok(SimpleVectorIndex::new())
    }

    /// Zapisuje indeks wektorowy do DB.
    fn save_graph(&self, gv: &SimpleVectorIndex) -> Result<()> {
