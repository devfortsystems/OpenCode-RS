...
async fn discover_antigravity() -> Result<(u16, String)> {
    ...
    let ps = tokio::process::Command::new("powershell")
        ...
        .kill_on_drop(true)
        .output();
    let cmd_output = tokio::time::timeout(Duration::from_secs(2), ps)
        .await
        .map_err(|_| anyhow!("PowerShell timeout przy wykrywaniu Antigravity"))?
    ...
    let port_ps = tokio::process::Command::new("powershell")
        .arg("-NoProfile")
        .arg("-Command")
        .arg(&port_cmd)
        .kill_on_drop(true)
        .output();
    let port_output = tokio::time::timeout(Duration::from_secs(2), port_ps)
        .await
        .map_err(|_| anyhow!("PowerShell timeout przy odczycie portów Antigravity"))?
