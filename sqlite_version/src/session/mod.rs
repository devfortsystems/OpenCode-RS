...
impl SessionManager {
    ...
    fn resolve_storage_dir(work_dir: &Path, storage_mode: &str) -> PathBuf {
        if storage_mode == "in_project" {
            let rs_dir = work_dir.join(".opencode-rs").join("sessions");
            if rs_dir.exists() {
                return rs_dir;
            }
            let legacy = work_dir.join(".opencode").join("sessions");
            if legacy.exists() {
                return legacy;
            }
            return rs_dir;
        ...
        // Domyślny tryb "central" — zapis w ~/.opencode-rs/projects/{sanitized}/sessions
        let path_str = work_dir.to_string_lossy().to_string();
        let sanitized = path_str.replace([':', '\\', '/'], "_");

        if let Some(user_dirs) = directories::UserDirs::new() {
            let rs_central = user_dirs.home_dir().join(".opencode-rs");
            return rs_central.join("projects").join(&sanitized).join("sessions");
        }
        ...
        work_dir.join(".opencode-rs").join("sessions")
    }
